...
from src.data.utils import load_mnist_dataset, load_omniglot_dataset
...
class TrainerQueryRefrenceSet(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.data=='mnist':
            ...
            train, test = load_omniglot_dataset('../../../../../')
...
class QueryRefrenceImageEncoder(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.optim_cfg.name == 'adam':
            optimizer = torch.optim.Adam(
                params, lr=self.optim_cfg.lr, weight_decay=self.optim_cfg.weight_decay)
        if self.optim_cfg.name == 'sgd':
            optimizer = torch.optim.SGD(params, lr=self.optim_cfg.lr, weight_decay=self.optim_cfg.weight_decay,
                                        momentum=self.optim_cfg.momentum, nesterov=self.optim_cfg.nesterov)
    ...
    def validation_epoch_end(self, plot_data):
        fig = plot_embeddings_unimodal(
            plot_data, self.current_epoch, return_fig=True)
...
def main(cfg: DictConfig):
    ...
    if cfg.data.name == 'mnist':
        ...
        transform1 = torchvision.transforms.Compose([
           torchvision.transforms.RandomRotation(15),
           torchvision.transforms.Normalize((0.9220,), (0.2516,))
        ])
        transform2 = torchvision.transforms.Compose([
           torchvision.transforms.RandomPerspective(0.5, 0.8),
           torchvision.transforms.Normalize((0.9220,), (0.2516,))
